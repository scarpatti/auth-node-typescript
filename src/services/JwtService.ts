...
export class JwtService {
  public static generateAccessToken = async (userId: string|number|null) => new Promise((resolve, reject) => {
    return userId && this.generateToken(
      {
        user_id: userId,
        access_token: true
      },
  ...
  public static generateToken = (payload: any, expiresIn: number | string, callback: any) => {
    return jwt.sign(payload, jwtConfig.JWT_KEY, {
      expiresIn: expiresIn
    }, callback);
  }

  public static revokeAccessToken = (token: string) => new Promise(async (resolve, reject) => {
    return await this.revokeToken(token, parseInt(jwtConfig.JWT_ACCESS_TOKEN_EXPIRED, 10))
      .then((response: any) => resolve(response))
      .catch((err: Error) => reject(err));
  })

  public static revokeRefreshToken = (token: string) => new Promise(async (resolve, reject) => {
    return await this.revokeToken(token, parseInt(jwtConfig.JWT_REFRESH_TOKEN_EXPIRED))
      .then((response: any) => resolve(response))
      .catch((err: Error) => reject(err));
  })

  public static revokeToken = async (token: string, expiresIn: number) => {
    return await jwtRepository.store(jwtConfig.JWT_REDIS_KEY_NAME + token, expiresIn, token);
  }

  public static verify = (token: string) => new Promise((resolve, reject) => {
    return jwt.verify(token, jwtConfig.JWT_KEY, (err: any, decoded: any) => {
      if (err) {
        reject(new Error(err.message));
      }
  ...
  public static verifyIsRevoked = async (token: string) => {
    return await jwtRepository.get(jwtConfig.JWT_REDIS_KEY_NAME + token);
  }
